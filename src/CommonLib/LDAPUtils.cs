...
public class LDAPUtils : ILDAPUtils {
  ...
  // The following byte stream contains the necessary message to request a
  // NetBios name from a machine
  ...
  private static readonly byte[] NameRequest = {
    0x80, 0x94, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
  ...
  private static readonly ConcurrentDictionary<
      string, ResolvedWellKnownPrincipal> SeenWellKnownPrincipals = new();

  private static readonly ConcurrentDictionary<string, byte> DomainControllers =
      new();
  ...
  private readonly ConcurrentDictionary<string, LdapConnection>
      _globalCatalogConnections = new();
  private readonly ConcurrentDictionary<string, string> _hostResolutionMap =
      new();
  private readonly ConcurrentDictionary<string, LdapConnection>
      _ldapConnections = new();
  private readonly ConcurrentDictionary<string, int> _ldapRangeSizeCache =
      new();
  ...
  /// <summary>
  ///     Creates a new instance of LDAP utils and allows overriding
  ///     implementations
  /// </summary>
  ...
  public LDAPUtils(NativeMethods nativeMethods = null,
                   PortScanner scanner = null, ILogger log = null) {
  ...
  public void SetLDAPConfig(LDAPConfig config) {
    _ldapConfig =
        config ?? throw new Exception("LDAP Configuration can not be null");
  ...
  /// <returns>True if a well known principal was identified, false if
  /// not</returns>
  public bool GetWellKnownPrincipal(string sid, string domain,
                                    out TypedPrincipal commonPrincipal) {
    if (!WellKnownPrincipal.GetWellKnownPrincipal(sid, out commonPrincipal))
      return false;
    var tempDomain = domain ?? GetDomain()?.Name ?? "UNKNOWN";
    commonPrincipal.ObjectIdentifier =
        ConvertWellKnownPrincipal(sid, tempDomain);
    SeenWellKnownPrincipals.TryAdd(
        commonPrincipal.ObjectIdentifier,
        new ResolvedWellKnownPrincipal { DomainName = domain, WkpId = sid });
    return true;
  }
  ...
  public IEnumerable<OutputBase> GetWellKnownPrincipalOutput(string domain) {
    foreach (var wkp in SeenWellKnownPrincipals) {
      WellKnownPrincipal.GetWellKnownPrincipal(wkp.Value.WkpId,
                                               out var principal);
      ...
      output.Properties.Add(
          "name",
          $"{principal.ObjectIdentifier}@{wkp.Value.DomainName}".ToUpper());
    ...
    entdc.Members =
        DomainControllers.Select(x => new TypedPrincipal(x.Key, Label.Computer))
            .ToArray();
    yield return entdc;
  }
  ...
  public string ConvertWellKnownPrincipal(string sid, string domain) {
    ...
    if (forest == null)
      _log.LogWarning("Error getting forest, ENTDC sid is likely incorrect");
    return $"{forest ?? "UNKNOWN"}-{sid}".ToUpper();
  }

  /// <summary>
  ///     Queries the global catalog to get potential SID matches for a username
  ///     in the forest
  /// </summary>
  ...
  public string[] GetUserGlobalCatalogMatches(string name) {
    ...
    var query =
        new LDAPFilter().AddUsers($"samaccountname={tempName}").GetFilter();
    var results = QueryLDAP(query, SearchScope.Subtree, new[] { "objectsid" },
                            globalCatalog: true)
                      .Select(x => x.GetSid())
                      .Where(x => x != null)
                      .ToArray();
  ...
  public TypedPrincipal ResolveIDAndType(string id, string fallbackDomain) {
    // This is a duplicated SID object which is weird and makes things unhappy.
    // Throw it out
    ...
    var type = id.StartsWith("S-") ? LookupSidType(id, fallbackDomain)
                                   : LookupGuidType(id, fallbackDomain);
    return new TypedPrincipal(id, type);
  }

  public TypedPrincipal ResolveCertTemplateByProperty(string propValue,
                                                      string propertyName,
                                                      string containerDN,
                                                      string domainName) {
    var filter = new LDAPFilter().AddCertificateTemplates().AddFilter(
        propertyName + "=" + propValue, true);
    var res = QueryLDAP(filter.GetFilter(), SearchScope.OneLevel,
                        CommonProperties.TypeResolutionProps,
                        adsPath: containerDN, domainName: domainName);
  ...
  public Label LookupSidType(string sid, string domain) {
    ...
    var result = QueryLDAP(CommonFilters.SpecificSID(sid), SearchScope.Subtree,
                           CommonProperties.TypeResolutionProps, rDomain)
                     .DefaultIfEmpty(null)
                     .FirstOrDefault();
  ...
  public Label LookupGuidType(string guid, string domain) {
    ...
    var result = QueryLDAP($"(objectguid={hex})", SearchScope.Subtree,
                           CommonProperties.TypeResolutionProps, domain)
                     .DefaultIfEmpty(null)
                     .FirstOrDefault();
  ...
  // public async IAsyncEnumerable<string> DoRangedRetrievalAsync(string
  // distinguishedName, string attributeName)
  // {
  //     var domainName = Helpers.DistinguishedNameToDomain(distinguishedName);
  //     LdapConnection conn;
  //     try
  //     {
  //         conn = await CreateLDAPConnection(domainName, authType:
  ...
  //     var complete = false;
  //
  //     var searchRequest = CreateSearchRequest($"{attributeName}=*",
  //     SearchScope.Base, new[] {currentRange},
  ...
  //         {
  //             searchResult = await
  //             Task.Factory.FromAsync(conn.BeginSendRequest,
  //             conn.EndSendRequest,
  //                 searchRequest,
  //                 PartialResultProcessing.NoPartialResultSupport, null);
  //         }
  //         catch (LdapException le) when (le.ErrorCode == 51 && retryCount <
  //         MaxRetries)
  //         {
  //             //Allow three retries with a backoff on each one if we get a
  //             "Server is Busy" error retryCount++; await
  //             Task.Delay(backoffDelay); backoffDelay =
  ...
  //         catch (Exception e)
  //         {
  //             _log.LogWarning(e,"Caught exception during ranged retrieval for
  //             {DN}", distinguishedName); yield break;
  //         }
  //
  //         if (searchResult is SearchResponse response &&
  ...
  //                 {
  //                     //Set our current range to the name of the attribute,
  //                     which will tell us how far we are in "paging"
  //                     currentRange = attr;
  //                     //Check if the string has the * character in it. If it
  //                     does, we've reached the end of this search complete =
  //                     currentRange.IndexOf("*", 0, StringComparison.Ordinal)
  //                     > 0;
  //                     //Set our step to the number of attributes that came
  //                     back. step = entry.Attributes[currentRange].Count;
  //                 }
  //             }
  //
  //
  ...
  ///     The function self-determines the range and internally handles the
  ///     maximum step allowed by the server
  /// </summary>
  /// <param name="distinguishedName"></param>
  /// <param name="attributeName"></param>
  /// <returns></returns>
  public IEnumerable<string> DoRangedRetrieval(string distinguishedName,
                                               string attributeName) {
    var domainName = Helpers.DistinguishedNameToDomain(distinguishedName);
    var task = Task.Run(
        () => CreateLDAPConnection(domainName, authType: _ldapConfig.AuthType));
    ...
    while (true) {
      ...
      try {
      ...
      } catch (LdapException le) when (
          le.ErrorCode == (int)LdapErrorCodes.Busy && retryCount < MaxRetries) {
        // Allow three retries with a backoff on each one if we get a "Server is
        // Busy" error
        retryCount++;
        ...
        _log.LogError(e, "Error doing ranged retrieval for {Attribute} on {Dn}",
                      attributeName, distinguishedName);
        yield break;
      }

      // If we ever get more than one response from here, something is horribly
      // wrong
      if (response?.Entries.Count == 1) {
        var entry = response.Entries[0];
        // Process the attribute we get back to determine a few things
        foreach (string attr in entry.Attributes.AttributeNames) {
          // Set our current range to the name of the attribute, which will tell
          // us how far we are in "paging"
          currentRange = attr;
          // Check if the string has the * character in it. If it does, we've
        ...
        foreach (string val in entry.Attributes[currentRange].GetValues(
                     typeof(string))) {
          yield return val;
          index++;
        }
  ...
  /// <summary>
  ///     Takes a host in most applicable forms from AD and attempts to resolve
  ///     it into a SID.
  /// </summary>
  ...
  public async Task<string> ResolveHostToSid(string hostname, string domain) {
    var strippedHost =
        Helpers.StripServicePrincipalName(hostname).ToUpper().TrimEnd('$');
    ...
    // Next we'll try calling NetWkstaGetInfo in hopes of getting the NETBIOS
    // name directly from the computer We'll use the hostname that we started
    // with instead of the one from our previous step
    ...
    if (RequestNETBIOSNameFromComputer(strippedHost, normalDomain,
                                       out tempName)) {
      tempDomain ??= normalDomain;
      tempName = $"{tempName}$".ToUpper();
  ...
  /// <summary>
  ///     Attempts to convert a bare account name (usually from session
  ///     enumeration) to its corresponding ID and object type
  /// </summary>
  /// <param name="name"></param>
  /// <param name="domain"></param>
  /// <returns></returns>
  public TypedPrincipal ResolveAccountName(string name, string domain) {
    ...
    if (Cache.GetPrefixedValue(name, domain, out var id) &&
        Cache.GetIDType(id, out var type))
      return new TypedPrincipal { ObjectIdentifier = id, ObjectType = type };

    var d = NormalizeDomainName(domain);
    var result = QueryLDAP($"(samaccountname={name})", SearchScope.Subtree,
                           CommonProperties.TypeResolutionProps, d)
                     .DefaultIfEmpty(null)
                     .FirstOrDefault();

    if (result == null) {
      _log.LogDebug("ResolveAccountName - unable to get result for {Name}",
                    name);
      return null;
    }
    ...
    if (id == null) {
      _log.LogDebug(
          "ResolveAccountName - could not retrieve ID on {DN} for {Name}",
          result.DistinguishedName, name);
      return null;
    }
  ...
  /// <summary>
  ///     Attempts to convert a distinguishedname to its corresponding ID and
  ///     object type.
  /// </summary>
  ...
  public TypedPrincipal ResolveDistinguishedName(string dn) {
    if (Cache.GetConvertedValue(dn, out var id) &&
        Cache.GetIDType(id, out var type))
      return new TypedPrincipal { ObjectIdentifier = id, ObjectType = type };

    var domain = Helpers.DistinguishedNameToDomain(dn);
    var result =
        QueryLDAP("(objectclass=*)", SearchScope.Base,
                  CommonProperties.TypeResolutionProps, domain, adsPath: dn)
            .DefaultIfEmpty(null)
            .FirstOrDefault();
    ...
    if (id == null) {
      _log.LogDebug(
          "ResolveDistinguishedName - could not retrieve object identifier from {DN}",
          dn);
      return null;
    }
  ...
  public IEnumerable<ISearchResultEntry> QueryLDAP(LDAPQueryOptions options) {
    return QueryLDAP(options.Filter, options.Scope, options.Properties,
                     options.CancellationToken, options.DomainName,
                     options.IncludeAcl, options.ShowDeleted, options.AdsPath,
                     options.GlobalCatalog, options.SkipCache,
  ...
  /// <param name="includeAcl">Include the DACL and Owner values in the
  /// NTSecurityDescriptor</param> <param name="showDeleted">Include deleted
  /// objects</param> <param name="domainName">Domain to query</param> <param
  /// name="adsPath">ADS path to limit the query too</param> <param
  /// name="globalCatalog">Use the global catalog instead of the regular LDAP
  /// server</param> <param name="skipCache">
  ///     Skip the connection cache and force a new connection. You must dispose
  ///     of this connection yourself.
  /// </param>
  /// <param name="throwException">Throw exceptions rather than logging the
  /// errors directly</param> <returns>All LDAP search results matching the
  /// specified parameters</returns> <exception cref="LDAPQueryException">
  ///     Thrown when an error occurs during LDAP query (only when
  ///     throwException = true)
  /// </exception>
  public IEnumerable<ISearchResultEntry>
  QueryLDAP(string ldapFilter, SearchScope scope, string[] props,
            CancellationToken cancellationToken, string domainName = null,
            bool includeAcl = false, bool showDeleted = false,
            string adsPath = null, bool globalCatalog = false,
            bool skipCache = false, bool throwException = false) {
    var queryParams =
        SetupLDAPQueryFilter(ldapFilter, scope, props, includeAcl, domainName,
                             includeAcl, adsPath, globalCatalog, skipCache);

    if (queryParams.Exception != null) {
      _log.LogWarning("Failed to setup LDAP Query Filter: {Message}",
                      queryParams.Exception.Message);
      if (throwException)
        throw new LDAPQueryException("Failed to setup LDAP Query Filter",
                                     queryParams.Exception);
      yield break;
    }
      ...
      try {
        ...
        if (response != null)
          pageResponse = (PageResultResponseControl)response.Controls
                             .Where(x => x is PageResultResponseControl)
                             .DefaultIfEmpty(null)
                             .FirstOrDefault();
      } catch (LdapException le)
          when (le.ErrorCode == (int)LdapErrorCodes.ServerDown &&
                retryCount < MaxRetries) {
        /*A ServerDown exception indicates that our connection is no longer
        valid for one of many reasons. However, this function is generally
        called by multiple threads, so we need to be careful in recreating the
        connection. Using a semaphore, we can ensure that only one thread is
        actually recreating the connection while the other threads that hit the
        ServerDown exception simply wait. The initial caller will hold the
        semaphore and do a backoff delay before trying to make a new connection
        which will replace the existing connection in the _ldapConnections
        cache. Other threads will retrieve the new connection from the cache
        instead of making a new one This minimizes overhead of new connections
        while still fixing our core problem.*/
        ...
        if (Monitor.TryEnter(_lockObj)) {
          // If we've acquired the lock, we want to immediately signal our reset
          // event so everyone else waits
          _connectionResetEvent.Reset();
          try {
            // Sleep for our backoff
            Thread.Sleep(backoffDelay);
            // Explicitly skip the cache so we don't get the same connection
            // back
            ...
            _log.LogInformation(
                "Created new LDAP connection after receiving ServerDown from server");
          } finally {
          ...
          // If someone else is holding the reset event, we want to just wait
          // and then pull the newly created connection out of the cache This
          // event will be released after the first entrant thread is done
          // making a new connection The thread.sleep is to prevent a potential,
          // very unlikely race
          Thread.Sleep(50);
        ...
        if (le.ErrorCode != (int)LdapErrorCodes.LocalError) {
          ...
          _log.LogWarning(
              ...
              le.ErrorCode, le.ServerErrorMessage, le.Message, ldapFilter,
              domainName);
        }
        ...
        _log.LogWarning(e, "Exception in LDAP loop for {Filter} and {Domain}",
                        ldapFilter, domainName);
        if (throwException)
          throw new LDAPQueryException(
              $"Exception in LDAP loop for {ldapFilter} and {domainName}", e);
  ...
  private LdapConnection CreateNewConnection(string domainName = null,
                                             bool globalCatalog = false,
                                             bool skipCache = false) {
    var task = globalCatalog
                   ? Task.Run(() => CreateGlobalCatalogConnection(
                                  domainName, _ldapConfig.AuthType))
  ...
  ///     throwException = true)
  /// </exception>
  public virtual IEnumerable<ISearchResultEntry>
  QueryLDAP(string ldapFilter, SearchScope scope, string[] props,
            string domainName = null, bool includeAcl = false,
            bool showDeleted = false, string adsPath = null,
            bool globalCatalog = false, bool skipCache = false,
            bool throwException = false) {
    return QueryLDAP(ldapFilter, scope, props, new CancellationToken(),
                     domainName, includeAcl, showDeleted, adsPath,
                     globalCatalog, skipCache, throwException);
  }

  private static TimeSpan GetNextBackoff(int retryCount) {
    return TimeSpan.FromSeconds(
        Math.Min(MinBackoffDelay.TotalSeconds *
                     Math.Pow(BackoffDelayMultiplier, retryCount),
                 MaxBackoffDelay.TotalSeconds));
  }
  ...
  /// <summary>
  ///     Tests the current LDAP config to ensure its valid by pulling a domain
  ///     object
  /// </summary>
  ...
  public bool TestLDAPConfig(string domain) {
    ...
    var result =
        QueryLDAP(filter.GetFilter(), SearchScope.Subtree,
                  CommonProperties.ObjectID, resDomain, throwException: true)
            .DefaultIfEmpty(null)
            .FirstOrDefault();
    _log.LogTrace("Result object from LDAP connection test is {DN}",
                  result?.DistinguishedName ?? "null");
    return result != null;
  }
  ...
  public virtual Domain GetDomain(string domainName = null) {
    ...
    try {
      ...
      if (_ldapConfig.Username != null)
        context = domainName != null
                      ? new DirectoryContext(DirectoryContextType.Domain,
                                             domainName, _ldapConfig.Username,
                                             _ldapConfig.Password)
                      : new DirectoryContext(DirectoryContextType.Domain,
                                             _ldapConfig.Username,
                                             _ldapConfig.Password);
      else
        context =
            domainName != null
      ...
      _log.LogDebug(e, "GetDomain call failed at {StackTrace}",
                    new StackFrame());
      domain = null;
    }
  ...
  /// <param name="props">LDAP properties to fetch for each object</param>
  /// <param name="includeAcl">Include the DACL and Owner values in the
  /// NTSecurityDescriptor</param> <param name="domainName">Domain to
  /// query</param> <param name="showDeleted">Include deleted objects</param>
  /// <param name="adsPath">ADS path to limit the query too</param>
  /// <param name="globalCatalog">Use the global catalog instead of the regular
  /// LDAP server</param> <param name="skipCache">
  ///     Skip the connection cache and force a new connection. You must dispose
  ///     of this connection yourself.
  /// </param>
  /// <returns>Tuple of LdapConnection, SearchRequest, PageResultRequestControl
  /// and LDAPQueryException</returns>
  // ReSharper disable once MemberCanBePrivate.Global
  internal LDAPQueryParams
  SetupLDAPQueryFilter(string ldapFilter, SearchScope scope, string[] props,
                       bool includeAcl = false, string domainName = null,
                       bool showDeleted = false, string adsPath = null,
                       bool globalCatalog = false, bool skipCache = false) {
    ...
    try {
      ...
      queryParams.Exception =
          new LDAPQueryException(errorString, ldapException);
      return queryParams;
    ...
    // If we get a null connection, something went wrong, but we don't have an
    // error to go with it for whatever reason
    if (conn == null) {
      var errorString =
    ...
    try {
      request = CreateSearchRequest(ldapFilter, scope, props, domainName,
                                    adsPath, showDeleted);
    } catch (LDAPQueryException ldapQueryException) {
      queryParams.Exception = ldapQueryException;
      return queryParams;
  ...
  private Group GetBaseEnterpriseDC(string domain) {
    ...
    g.Properties.Add(
        "name",
        $"ENTERPRISE DOMAIN CONTROLLERS@{forest ?? "UNKNOWN"}".ToUpper());
  ...
  /// </summary>
  /// <param name="config"></param>
  public void UpdateLDAPConfig(LDAPConfig config) { _ldapConfig = config; }
  ...
  private string GetDomainNameFromSidLdap(string sid) {
    ...
    var result = QueryLDAP($"(&(objectclass=domain)(objectsid={hexSid}))",
                           SearchScope.Subtree, new[] { "distinguishedname" },
                           globalCatalog: true)
                     .DefaultIfEmpty(null)
                     .FirstOrDefault();
    ...
    if (result != null) {
      var domainName =
          Helpers.DistinguishedNameToDomain(result.DistinguishedName);
      return domainName;
    }

    // Try trusteddomain objects with the securityidentifier attribute
    result =
        QueryLDAP($"(&(objectclass=trusteddomain)(securityidentifier={sid}))",
                  SearchScope.Subtree, new[] { "cn" }, globalCatalog: true)
            .DefaultIfEmpty(null)
            .FirstOrDefault();
  ...
  /// <summary>
  ///     Uses a socket and a set of bytes to request the NETBIOS name from a
  ///     remote computer
  /// </summary>
  ...
  /// <returns></returns>
  private static bool RequestNETBIOSNameFromComputer(string server,
                                                     string domain,
                                                     out string netbios) {
    var receiveBuffer = new byte[1024];
    var requestSocket = new Socket(AddressFamily.InterNetwork, SocketType.Dgram,
                                   ProtocolType.Udp);
    try {
      // Set receive timeout to 1 second
      requestSocket.SetSocketOption(SocketOptionLevel.Socket,
      ...
      if (IPAddress.TryParse(server, out var parsedAddress))
        ...
        try {
          IPAddress address;
          if (server.Contains("."))
            address = Dns.GetHostAddresses(server).First(
      ...
      try {
        ...
        var receivedByteCount =
            requestSocket.ReceiveFrom(receiveBuffer, ref remoteEndpoint);
        if (receivedByteCount >= 90) {
          netbios = new ASCIIEncoding()
                        .GetString(receiveBuffer, 57, 16)
                        .Trim('\0', ' ');
          return true;
        }
  ...
  /// <param name="hostname"></param>
  /// <returns></returns>
  private async Task<NetAPIStructs.WorkstationInfo100?>
  GetWorkstationInfo(string hostname) {
  ...
  private SearchRequest CreateSearchRequest(string filter, SearchScope scope,
    ...
    var adPath =
        adsPath?.Replace("LDAP://", "") ?? $"DC={domain.Replace(".", ",DC=")}";
  ...
  /// <param name="authType">Auth type to use. Defaults to Kerberos. Use
  /// Negotiate for netonly scenarios</param> <returns>A connected
  /// LdapConnection or null</returns>
  private async Task<LdapConnection>
  CreateGlobalCatalogConnection(string domainName = null,
    ...
    if (targetServer == null)
      throw new LDAPQueryException(
          $"No usable global catalog found for {domainName}");

    if (_globalCatalogConnections.TryGetValue(targetServer, out var connection))
      return connection;

    connection =
        new LdapConnection(new LdapDirectoryIdentifier(targetServer, 3268));

    connection.SessionOptions.ProtocolVersion = 3;

    if (_ldapConfig.Username != null) {
      var cred =
          new NetworkCredential(_ldapConfig.Username, _ldapConfig.Password);
  ...
  /// <summary>
  ///     Creates an LDAP connection with appropriate options based off the ldap
  ///     configuration. Caches connections
  /// </summary>
  /// <param name="domainName">The domain to connect too</param>
  /// <param name="skipCache">Skip the connection cache</param>
  /// <param name="authType">Auth type to use. Defaults to Kerberos. Use
  /// Negotiate for netonly scenarios</param> <returns>A connected LDAP
  /// connection or null</returns>
  private async Task<LdapConnection>
  CreateLDAPConnection(string domainName = null, bool skipCache = false,
                       AuthType authType = AuthType.Kerberos) {
    string targetServer;
    if (_ldapConfig.Server != null)
      targetServer = _ldapConfig.Server;
      ...
      if (domain == null) {
        _log.LogDebug(
            "Unable to create ldap connection for domain {DomainName}: GetDomain failed",
            domainName);
        throw new LDAPQueryException(
    ...
    if (targetServer == null)
      throw new LDAPQueryException(
          $"No usable domain controller found for {domainName}");
    ...
    var connection =
        new LdapConnection(ident) { Timeout = new TimeSpan(0, 0, 5, 0) };
    if (_ldapConfig.Username != null) {
      var cred =
          new NetworkCredential(_ldapConfig.Username, _ldapConfig.Password);
      connection.Credential = cred;
    ...
    if (_ldapConfig.DisableCertVerification)
      connection.SessionOptions.VerifyServerCertificate =
          (ldapConnection, certificate) => true;

    connection.AuthType = authType;

    _ldapConnections.AddOrUpdate(targetServer, connection,
                                 (s, ldapConnection) => {
  ...
  private async Task<string> GetUsableDomainController(Domain domain,
                                                       bool gc = false) {
    if (!gc &&
        _domainControllerCache.TryGetValue(domain.Name.ToUpper(), out var dc))
      return dc;
    ...
    if (await _portScanner.CheckPort(pdc, port)) {
      ...
      _log.LogInformation("Found usable Domain Controller for {Domain} : {PDC}",
                          domain.Name, pdc);
      return pdc;
    }
    ...
    foreach (DomainController domainController in domain.DomainControllers) {
      ...
      if (!await _portScanner.CheckPort(name, port))
        continue;
      _log.LogInformation("Found usable Domain Controller for {Domain} : {PDC}",
                          domain.Name, name);
    ...
    _log.LogWarning("Unable to find usable domain controller for {Domain}",
                    domain.Name);
    return null;
  }
  ...
  /// <summary>
  ///     Turns a domain Netbios name into its FQDN using the DsGetDcName
  ///     function (TESTLAB -> TESTLAB.LOCAL)
  /// </summary>
  /// <param name="domainName"></param>
  /// <returns></returns>
  ...
  public int GetDomainRangeSize(string domainName = null,
                                int defaultRangeSize = 750) {
    var domainPath = DomainNameToDistinguishedName(domainName);
    // Default to a page size of 750 for safety
    if (domainPath == null) {
      _log.LogDebug(
          "Unable to resolve domain {Domain} to distinguishedname to get page size",
          domainName ?? "current domain");
      return defaultRangeSize;
    }

    if (_ldapRangeSizeCache.TryGetValue(domainPath.ToUpper(),
                                        out var parsedPageSize)) {
      return parsedPageSize;
    }

    var configPath =
        CommonPaths.CreateDNPath(CommonPaths.QueryPolicyPath, domainPath);
    var enumerable = QueryLDAP("(objectclass=*)", SearchScope.Base, null,
                               adsPath: configPath);
    ...
    if (int.TryParse(pageSize.Split('=').Last(), out parsedPageSize)) {
      ...
      _log.LogInformation("Found page size {PageSize} for {Domain}",
                          parsedPageSize, domainName ?? "current domain");
      return parsedPageSize;
    }

    _log.LogDebug("Failed to parse pagesize for {Domain}, returning default",
                  domainName ?? "current domain");
  ...
  private string DomainNameToDistinguishedName(string domain) {
    ...
    return resolvedDomain == null ? null
                                  : $"DC={resolvedDomain.Replace(".", ",DC=")}";
  }
  ...
  public string GetConfigurationPath(string domainName = null) {
    ...
    if (_ldapConfig.Username != null)
      rootDse =
          new DirectoryEntry(path, _ldapConfig.Username, _ldapConfig.Password);
    else
  ...
  public bool IsDomainController(string computerObjectId, string domainName) {
    var filter =
        new LDAPFilter()
            .AddFilter(LDAPProperties.ObjectSID + "=" + computerObjectId, true)
